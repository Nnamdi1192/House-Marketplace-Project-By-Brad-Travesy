...
const EditListing = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try{
            ...
            setLoading(false);
            navigate(`/single-listing/${type}/${params.listingId}`);
        }catch(err){
            setLoading(false);
    ...
    // Get the auth ID of the current user when available
    useEffect(() => {
        if(isMounted){
            onAuthStateChanged(auth, (user) => {
                if(user){
                    // Populate the form data with authorized users Id as user ref
    ...
    // Redirect user if user is not owner of listing
    useEffect(() => {
        if(listings && listings.userRef !== auth.currentUser.uid){
            toast.error('You do not have the permission to edit this listing');
            navigate('/');
        }
    }, [auth, listings, navigate]);
